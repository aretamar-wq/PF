    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            elseif ($method -eq 'GET' -and $path -eq '/api/output-files') {
                ...
                # (pfout-/pfouterror-/dbnout-/dbnouterror-/dbnconsulta- + 14 dígitos
                # + .csv) que valida ese endpoint al guardar, acá para filtrar
                # cualquier otra cosa que haya en la carpeta.
                $items = @()
                if (Test-Path $filesDir) {
                    $items = @(Get-ChildItem -Path $filesDir -File |
                        Where-Object { $_.Name -match '^(pfout|pfouterror|dbnout|dbnouterror|dbnconsulta)-\d{14}\.csv$' } |
            ...
            elseif ($method -eq 'GET' -and $path -eq '/api/output-files/content') {
                ...
                if ($name -notmatch '^(pfout|pfouterror|dbnout|dbnouterror|dbnconsulta)-\d{14}\.csv$') {
