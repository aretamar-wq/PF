...
`wwwroot/`, los mismos `Flows/*.json` y la misma base MariaDB (ver "Base de
datos (MariaDB)" más abajo) — el frontend no sabe ni le importa cuál de los
dos tiene enfrente. Elegí uno, **no hace falta correr los dos a la vez** en
el mismo servidor:
...
3. Tener una base MariaDB 10 accesible (en el mismo server o en otro),
   ...
   **raíz del repo** con los datos de conexión — ver "Base de datos
   ...
   usuarios/roles, config de AD, perfiles de conexión y parametría —
   la misma base que usa el backend PowerShell, no hace falta una por
   backend.
...
nada). Con el backend Node.js (instala `mysql2` de paso):
...
O con el backend PowerShell (no requiere Node.js, mismo comportamiento):

```powershell
pwsh modules/scripts/Migrate-JsonToMariaDb.ps1
```

Cualquiera de los dos scripts migra las 4 fuentes (`security.local.json`,
`profiles.local.json`, `parametria.local.json`,
`logs/processed-operations.json`) — no hace falta correr los dos, alcanza
con uno. Son seguros de correr más de una vez (usuarios/perfiles se
upsertean por su clave, las operaciones procesadas tienen una `UNIQUE KEY`
que evita duplicados) — no borran ni modifican los archivos `*.local.json`
...
de la tabla `perfiles` (ver "Backend Node.js (`node/`)" / "Backend
PowerShell (`modules/`)") y hoy solo se completan con un `UPDATE` directo a
MariaDB, o migrando un `profiles.local.json` viejo que ya los tuviera (ver
"Base de datos (MariaDB)").
